...
- skill_package_version: `2.4.1`
- task_package_version: `2.4.1`
...
- 全Taskは `Required Skill Runtime=Txx@2.4.1`・active Skill runtime・`05_CONFIG.txx_required_skill_version=2.4.1` の3点一致を実行前に確認する。旧runtime/確認不能はE024で書込前FAIL CLOSED。
...
Spark Runtime 2.4.1: 全Task runtime pin、logical-key exact-row、Task専用log lane、Sheet-owned cursor、literal heartbeat target+guard、exact-row postwrite readback、scheduled pure NOOP heartbeat-only、terminal workset recountを共通化。
